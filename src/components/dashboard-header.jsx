    ...
    <header className="h-[68px] border-b border-border flex items-center justify-between px-4 lg:px-6 shrink-0 bg-background transition-colors">
      ...
      <div className="flex items-center gap-3 lg:gap-4">
        ...
        <div className="flex items-center gap-2 text-[14px]">
          ...
          <span className="text-muted-foreground/90 font-medium">{view}</span>
        </div>
      </div>
...
function HeaderIcon({ icon, onClick }) {
  return (
    <button
      onClick={onClick}
      className="p-2 text-foreground hover:text-foreground hover:bg-accent rounded-md transition-all"
    >
      {icon}
    </button>
  )
